...
def main(input_dir: str, output_dir: str, datajson):
    ...
    for row in tqdm(files):
        ...
        if out_path.replace('.m4a', '.wav') not in exits:
            try:
                os.system(
                    f"cp -r {input_dir}/{row['audio']} {output_dir}/{out_path}")
                os.system(
                    f"ffmpeg -i {output_dir}/{out_path} {output_dir}/{out_path.replace('.m4a', '.wav')} -loglevel panic -y")
...
if __name__ == "__main__":
    ...
    args.add_argument("--output_path", type=str, default="./dataset/audios")
