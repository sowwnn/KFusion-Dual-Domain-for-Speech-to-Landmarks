...
class S2LM(L.LightningModule):
    def __init__(self, batch, init_lr, duration, num_of_landmarks=478):
        ...
        self.linear = nn.Linear(in_features=768, out_features=num_of_landmarks*2)
        ...
        self.lstm_m = nn.LSTM(input_size=768, hidden_size=256, num_layers=16, batch_first=True, bidirectional=True)

        self.output_mapper_m = nn.Linear(256*2, len(self.MOUTH_LANDMARKS)*2)
    ...
    def forward(self, x, v, w):
        ...
        v[:,:,self.MOUTH_LANDMARKS] = 0
        v = v.view(self.batch,1, -1)
        v = self.conv(v)

        k = self.output_mapper_m(k)
